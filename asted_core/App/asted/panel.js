document.addEventListener('DOMContentLoaded', function () {
	...
	if (localStorage.getItem("asted-panel-fixed") === null) {
		localStorage.setItem("asted-panel-fixed", "no-fixed");
	}
	if (localStorage.getItem("asted-panel-collapse") === null) {
		localStorage.setItem("asted-panel-collapse", "no-collapsed");
	}
	document.querySelector('.asted-admin-panel-wrapper').style.marginBottom='0';
	...
	const astedAdminPAnelHeight =
		...
		//ASTED: Если сохранено что :: панель открыта
		if (localStorage.getItem("asted-panel-collapse") === "no-collapsed") {
			console.log("no-collapsed");
			collapseButton.querySelector('p').textContent='Развернуть';
			collapseButton.lastElementChild.classList.remove("asted-arrow-clicked");
		}
		//ASTED: Если сохранено что :: панель закрыта
		if (localStorage.getItem("asted-panel-collapse") === "collapsed") {
			...
			collapseButton.querySelector('p').textContent='Свернуть';
			collapseButton.lastElementChild.classList.toggle("asted-arrow-clicked");
		}
	collapseButton.addEventListener("click", () => {

		if(astedPanel.classList.contains('asted-collapse-admin-panel')){
			collapseButton.querySelector('p').textContent='Свернуть';
		}else{
			collapseButton.querySelector('p').textContent='Развернуть';
		}
