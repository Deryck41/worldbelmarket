...
$imgset = array(
    ...
    'minwidth' => 10,
    'minheight' => 10,
    'type' => array('bmp', 'gif', 'webp', 'jpg', 'jpeg', 'png'),
);
...
if (isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
    ...
    if (in_array($type, $imgset['type'])) {
        ...
        if (isset($width) && isset($height)) {
            ...
            if ($width < $imgset['minwidth'] || $height < $imgset['minheight']) {
                $re .= '\\n Width x Height = ' . $width . ' x ' . $height . '\\n ASTED Минимальная ширина x высота должна быть: ' . $imgset['minwidth'] . ' x ' . $imgset['minheight'];
            }
        ...
        $re .= 'The file: ' . $_FILES['upload']['name'] . ' ASTED: не имеет разрешенного типа расширения.';
    }
    ...
    if ($re == '') {
        if (move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
            ...
            $url = '/asted/' . $upload_dir . $f_name;
            $msg = F_NAME . '.' . $type . ' ASTED: успешно загружено: \\n- Size: ' . number_format($_FILES['upload']['size'] / 1024, 2, '.', '') . ' KB';
            ...
            $re = '<script>alert("ASTED: Не удалось загрузитьfile")</script>';
        }
    } else {
