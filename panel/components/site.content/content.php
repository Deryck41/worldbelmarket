...
if ($_POST['submit'] == 'menuadd') {
  ...
  if (mysqli_query($link, $sql)) {
    echo '<meta http-equiv="refresh" content="0;URL=/panel/content/' . $_GET['id'] . '/" />';
  } else {
...
<div class="container-fluid">
  ...
  while ($region = mysqli_fetch_assoc($result_region)) {
    ...
    $CatalogRegionFilter[] = '<div id="filter_' . $astedRegionId . '" class="top-filter__input">
                   ...
                   ';
    $CatalogRegionFilterScript2[] = '<script>$("#filter_' . $astedRegionId . '").click(function () {window.location.href = "/panel/content/' . $astedRegionId . '/";});</script>';
  }
  ?>
    ...
    ?>
        <div class="card-body">
          <div class="titleblock">
            ...
            <div><i onClick="copyCode(this)" class="contenttitle" title="Скопировать"  style="cursor: pointer; color: rgb(102, 102, 92);">{$BLOCK<?= $block ?>}</i> <a href="/panel/content-edit/<?= $idmenu ?>/" style="    padding-right: 12px;"><i class="fas fa-fw fa-edit" data-toggle="modal" data-target="#myModalka1"></i></a><i class="fas fa-fw fa-trash trashclick<?= $idmenu ?>" value="<?= $idmenu ?>" style="float: right;"></i>
            </div>
          </div>
...
<script>
  ...
  $("#filter_0").click(function() {
    window.location.href = "/panel/content/";
  });
</script>
<script>
