...
if($userSessionDivisions == "1"){
	...
	$sql = "INSERT INTO `crm_site_section` (forwebsite, namesection, websitemodule) VALUES ('{$_GET['id']}','{$_POST['websitesectname']}', 'custom')";
    ...
    echo '<meta http-equiv="refresh" content="0;URL='.$astedADM.'section/1/13059/" />';
        ...
        TerranCRM: Ошибка добавления записи!!!<br> Запрос в базу: '.$sql.' <br> Ошибка: '.mysqli_error($link).'
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Секции меню</h1>
           </div>
		  <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">Список секций меню</h6>
                </div>
                <div class="card-body">
				<?while ($task = mysqli_fetch_assoc($result_sitesectionfor)) {$forsite = "{$task['forwebsite']}";$websitemodule = "{$task['websitemodule']}";$namesection = "{$task['namesection']}";$id = "{$task['id']}";	
					?>
				<div class="p-3 bg-gray-100 websiteadd">ID Section - Название секции</div>
                  <?if($websitemodule == "custom"){?><div class="p-3 bg-gray-100">#<?=$id?> - <?=$namesection?></div><br><?}?>
				<?}?>
				...
				<form action="" method="post">
						...
						<input type="submit" name="submit" style="display:none" value="websiteadd" name="websiteadd" id="id0"
