...
<div class="container-fluid">
  ...
  if ($_POST['submit'] == "menuadd") {
    ...
    if (R::store($categoryAdd)) {
      if ($_GET['result'] == 'podcategory') {
        echo '<meta http-equiv="refresh" content="0;URL=/panel/category/' . $_GET['id'] . '/podcategory/" />';
      } else {
        echo '<meta http-equiv="refresh" content="0;URL=/panel/category/' . $_GET['id'] . '/" />';
      }
    } else {
              ...
              <form action="" method="post" class="container-fluid">
                <div class="row">
                  ...
                  <div class="<?php if (!empty($images)) { ?>col-7<? } else { ?>col-8<? } ?>">
                    <a href="/panel/category/<?= $id ?>/podcategory/" style="font-size: 14px;color: currentcolor;"><?= $name ?></a>
                    <div style="font-size: 12px;"><?= $link ?></div>
                  </div>
                  <div class="col-4"> <i class="fas fa-fw fa-trash trashclick" onclick="Delete(<?= $id ?>,'category')" value="<?= $id ?>" style="float: right;"></i> <a href="/panel/category-edit/<?= $id ?>/" style="    padding-right: 12px;"><i class="fas fa-fw fa-edit" data-toggle="modal" data-target="#myModalka<?= $id ?>" style="float: right;"></i></a>
                  </div>
                </div>
              </form><br>
            <? } ?>
