          ...
          if (strpos($file, ".php") !== false) { //проверяем есть ли пхп что-бы не создавать роуты для css, js и т.д
              ...
              if (mysqli_query($link, $sql)) {
                ...
                echo '<div class="container-fluid"><div class="alert alert-warning" role="alert">
                  Ошибка добавления записи!!!<br> Запрос в базу: ' . $sql . ' <br> Ошибка: ' . mysqli_error($link) . '
              </div></div>';
              }
          }
        }
...
">
                <div class="col-8">
                <strong>URI:</strong> <a href="/panel/<?= $uri ?>/"><?= $uri ?></a> <strong>Direction:</strong><?= str_replace("asted.", "", $direction) ?> <strong>Page_name:</strong><?= $link ?>
                <div>
              <?php
$filename = '../panel/components/'.$direction.'/info.asted';
...
<div class="col-4">
              ...
              <script>
                $('.trashclick<?= $idmenu ?>').click(function() {
                  let confirmation = confirm("Вы уверены, что хотите удалить роутинг компанента?");
                  if (confirmation) {
                    var menu_id = "<?= $idmenu ?>";
                    $.ajax({
...
document.querySelectorAll(".update-form").forEach(function(form) {
    form.addEventListener("submit", function (event) {
        ...
        fetch("/panel/components/asted.acom/update_component.php", {
            method: "POST",
            body: formData,
        })
