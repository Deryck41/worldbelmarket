...
<div class="container-fluid">
    ...
    if ($_POST['submit'] == "menuadd") {
        ...
        if (R::store($filterAdd)) {
            if ($_GET['result'] == 'filvalue') {
                echo '<meta http-equiv="refresh" content="0;URL=/asted/category-filter/' . $_GET['id'] . '/filvalue/" />';
            } else {
                echo '<meta http-equiv="refresh" content="0;URL=/asted/category-filter/' . $_GET['id'] . '/" />';
            }
        } else {
        ...
        <div class="col-lg-12 mb-4">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    ...
                    <? if (!empty($_GET["result"])) {
                        echo '<div style="padding: 5px;"><a href="/asted/category-filter/' . $mainFilter['forcatalog'] . '/" class="btsbackconedit">⇇ Вернутся назад</a></div>';
                    } ?>
                    ...
                    <div style="padding-top: 2px;">
                        ...
                        ?>
                            <form action="" method="post">
                                <? if (empty($_GET['result'])) { ?>
                                    <a href="/asted/category-filter/<?= $id ?>/filvalue/" style="font-size: 12px;color: currentcolor;"><?= $name ?><? if ($categoryCount != $countCategory) {
                                                                                                                                                        echo ' (' . implode(', ', $categoryName) . ')';
                                                                                                                                                    } ?></a>
                                <? } else {
                                    echo $name;
                                } ?>
                                <i class="fas fa-fw fa-trash trashclick" onclick="Delete(<?= $id ?>,'category-filter')" value="<?= $id ?>" style="float: right;"></i> <a href="/asted/category-filter-edit/<?= $id ?>/" style="    padding-right: 12px;"><i class="fas fa-fw fa-edit" data-toggle="modal" data-target="#myModalka<?= $id ?>" style="float: right;"></i></a>
                            </form><br>
                        <? } ?>
                    </div>
