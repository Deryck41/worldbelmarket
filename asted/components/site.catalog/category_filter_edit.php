...
<div class="container-fluid">
	...
	if ($_POST['submit'] == 'websiteadd') {
		...
		if (R::store($filterUpdate)) {
			echo '<meta http-equiv="refresh" content="0;URL=/asted/category-filter-edit/' . $_GET['id'] . '/" />';
		} else {
	...
	if (empty($site_object['parent'])) {
		...
		echo '<div style="padding: 5px;"><a href="/asted/category-filter/' . $site_object['forcatalog'] . '/" class="btsbackconedit">&#8647; Вернутся</a></div>';
	} else {
		$site_objectMain = R::load('crm_site_catalog_filter', $site_object['parent']);
		echo '<div style="padding: 5px;"><a href="/asted/category-filter/' . $site_object['parent'] . '/filvalue/" class="btsbackconedit">&#8647; Вернутся</a></div>';
	}
	?>
